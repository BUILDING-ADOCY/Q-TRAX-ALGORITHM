...
async def poll_http_result(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            async with session.get(f"{api_url}/job/{job_id}/result") as resp:
                if resp.status == 200:
                ...
                txt = await resp.text()
                logging.error("Unexpected %s for job %s: %s", resp.status, job_id, txt)
                return None
        except Exception:
...
async def send_random_scenario(
    ...
    # enqueue
    try:
        async with session.post(f"{api_url}/enqueue_dynamic", json=payload) as resp:
            resp.raise_for_status()
            data: Dict[str, Any] = await resp.json()
            job_id = data.get("job_id")
            if not job_id:
                logging.error("No job_id in response: %s", data)
                return
    except Exception:
    ...
    # poll
    result = await poll_http_result(session, api_url, job_id, http_retries, http_base_delay)
    logging.info("[HTTP] Job %s => %s", job_id, result.get("routes") if result else None)
...
async def generate_traffic(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        while True:
            tasks = [
                asyncio.create_task(
                    send_random_scenario(session, api_url, http_retries, http_base_delay, save_path)
                )
