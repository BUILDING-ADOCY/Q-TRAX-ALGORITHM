...
from src.qtrax.models.problem import Problem, Node, Edge
from src.qtrax.models.solution import Solution



def load_problem_yaml(path: str) -> Problem:
    """
    ...
    nodes = [Node(item["id"], item.get("attributes")) for item in nodes_data]
    edges = [
        Edge(item["source"], item["target"], item["weight"], item.get("attributes"))
        for item in edges_data
    ]

    return Problem(nodes=nodes, edges=edges, constraints=constraints)


def save_solution_json(solution: Solution, path: str) -> None:
    """
